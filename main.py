...
def _summarize_trials(trials: list[dict], fallback_points: int = 100) -> dict[str, float | int]:
    ...
    quality_rate = sum(1 for v in quality_values if _as_bool(v)) / len(quality_values) if quality_values else 0.0

    win_values = [row.get("won", None) for row in trials if row.get("won", None) is not None]
    win_rate = sum(1 for v in win_values if _as_bool(v)) / len(win_values) if win_values else 0.0
    ...
    color_rt_values = [_as_float(row.get("color_rt_s", row.get("color_rt", None))) for row in trials]
    ...
    bet_rt_values = [_as_float(row.get("bet_rt_s", row.get("bet_rt", None))) for row in trials]
...
def run(options: TaskRunOptions):
    """Run Cambridge Gambling Task in human/qa/sim mode with one auditable flow."""
